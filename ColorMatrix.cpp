...
ColorMatrix ColorMatrix::generateQuadtree(double levelOfDetails, int maxSize, int minSize, int zeros)
{
	...
	std::cout << countRows << "x" << countCols;
	...
	if (!doSplit){
		...
		ColorMatrix P = ColorMatrix(Matrix<Color>(countRows,countCols,avgColor));
		if (zeros > -1){ // do polynomial split only if zeros > -1
			P = polynomialSplit(zeros);
		}
		// calc distance
		Matrix<double> distToAvg = P.distance(*this);
		double avg = distToAvg.average();
 ...
 */
ColorMatrix ColorMatrix::polynomialSplit(int zeros)
{
	Matrix<int> output(countRows,countCols,0);
	...
	for (int k = 0; k < countCols; k++){ // loop column wise through the image
		...
		// 	std::cout << (progress*10) << "%" << std::endl;
		// }

		// DEBUG
		// for (int l = 0; l < minCutPos; l++){
		// 	output.setElement(l,k,1);
		// }
	}

	// generate matrix A (needed for the system of linear equations)
	Matrix<double> A(countCols,zeros+1,1);
	...
	for (int x = 0; x < countCols; x++){
		...
		for (int y = 0; y < fx && y < countRows; y++){
	...
	Matrix<Color> areaB(countRows*countCols-countPixelsInA,1,Color(0,0,0));
	int aCount = 0;
	int bCount = 0;
	for (int i = 0; i < countRows; i++){
		for (int j = 0; j < countCols; j++){
